...
class MovieDatabase {
  ...
  Future<Database> initDB() async {
    ...
    var theDb = await openDatabase(path, version: 1, onCreate: _onCeate);
    return theDb;
  }

  void _onCeate(Database db, int version) async {
    await db.execute(
  ...
  Future<int> deleteMovie(String id) async {
    var dbClient = await db;
    var res =
        await dbClient.delete("Movies", where: "id = ? ", whereArgs: [id]);
